...
void sprint_worker( const std::vector <int>& v, std::vector <int>::const_iterator& begin_it, int target, std::atomic <double>& result )
{
   if ( begin_it == v.end() ) { result = 0.0; return; }

   auto pos = std::find( begin_it, v.end(), target );
   auto end = ( ( pos != v.end() ) ? pos + 1 : v.end() );
   ...
   result = static_cast <double> (std::accumulate( begin_it, end, 0 )) / std::distance( begin_it, end );
...
int main( int argc, char* argv[] ) {
   ...
   std::vector < std::vector <int>::const_iterator > current_positions;
   for (int i = 0; i < THREADS; ++i)
      current_positions.emplace_back( data_sets[i].begin() );
   ...
   for (int iteration = 0; iteration < ITERATIONS; ++iteration) {
      ...
      for (int i = 0; i < THREADS; ++i)
         threads.emplace_back( sprint_worker, std::cref( data_sets[i] ), std::ref( current_positions[i] ), target, std::ref( results[i] ) );
